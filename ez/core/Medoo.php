...
class Medoo
{
	...
	public function connect($type = 1)
	{
		...
		try {
			...
			if (isset($options[ 'dsn' ])) {
				...
				switch ($this->database_type) {
					...
					case 'mysql':
						...
						/* 使用双引号创建sql (Make MySQL using standard quoted identifier) */ 
					...
					case 'oracle':
						$attr = [
							'driver' => 'oci',
							'dbname' => $options[ 'dbHost' ] ?
								'//' . $options[ 'dbHost' ] . ($is_port ? ':' . $port : ':1521') . '/' . $options[ 'dbName' ] :
								$options[ 'dbName' ]
						];

						if (isset($options[ 'dbCharset' ])) {
							$attr[ 'charset' ] = $options[ 'dbCharset' ];
						}
					...
					case 'mssql':
						if (strstr(PHP_OS, 'WIN')) {
							$attr = [
								'driver' => 'sqlsrv',
								'Server' => $options[ 'dbHost' ] . ($is_port ? ',' . $port : ''),
								'Database' => $options[ 'dbName' ]
							];
						} else {
							$attr = [
								'driver' => 'dblib',
								'host' => $options[ 'dbHost' ] . ($is_port ? ':' . $port : ''),
								'dbname' => $options[ 'dbName' ]
							];
						}
					...
					case 'sqlite':
                        if (!config('dbDistributede') || $type == 1  || $this->transaction) {
                            ...
                            $_link[1] = new PDO('sqlite:' . $options[ 'dbFile' ], null, null, $this->option);
                            return $_link[1];
                        }
				}
			}
			...
			if (in_array($this->database_type, ['mariadb', 'mysql', 'pgsql', 'sybase', 'mssql']) && isset($options[ 'dbCharset' ])) {
	...
	public function query($query, $map = [])
	{
		...
		return $this->exec($link, $query, $map);
	}

	public function exec($link, $query, $map = [])
	{
		if ($this->debug_mode) {
	...
	protected function arrayQuote($array)
	{
		...
		{
			$stack[] = is_int($value) ? $value : $this->connect()->quote($value);
		}
	...
	public function update($table, $data, $where = null)
	{
		...
		return $this->exec($this->connect(), 'UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $fields) . $this->whereClause($where, $map), $map);
	}
	...
	public function delete($table, $where)
	{
		...
		return $this->exec($this->connect(), 'DELETE FROM ' . $this->tableQuote($table) . $this->whereClause($where, $map), $map);
	}
	...
	public function replace($table, $columns, $where = null)
	{
		...
		return $this->exec($this->connect(), 'UPDATE ' . $this->tableQuote($table) . ' SET ' . $replace_query . $this->whereClause($where, $map), $map);
	}
